...
text = one(text, """    pub max_items: usize,
    pub max_data_components: usize,
""", """    pub max_items: usize,
    pub max_entity_types: usize,
    pub max_data_components: usize,
""", "entity limit field")
text = one(text, """            max_items: 100_000,
            max_data_components: 100_000,
""", """            max_items: 100_000,
            max_entity_types: 100_000,
            max_data_components: 100_000,
""", "entity limit default")
text = one(text, """    /// Static item registry IDs used by Play-state ItemStack codecs.
...
""", """    /// Static item registry IDs used by Play-state ItemStack codecs.
    ...
    /// Static data-component-type IDs used by version-aware ItemStack codecs.
""", "entity pack field")
text = one(text, """#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
text = one(text, """            data_components: Vec::new(),
            packet_catalog: vec![
""", """            entity_types: vec![
                RomPackEntityType {
...
text = one(text, """        let mut pack = sample_pack();
...
text = path.read_text()
text = one(text, "use ferrum_rompack::{RomPackDataComponent, RomPackItem};", "use ferrum_rompack::{RomPackDataComponent, RomPackEntityType, RomPackItem};", "registry report import")
text = one(text, """pub struct RegistryProtocolReport {
    pub items: Vec<RomPackItem>,
...
text = one(text, """    let data_components = parse_registry(&root, "minecraft:data_component_type")?
...
text = one(text, """    Ok(RegistryProtocolReport {
        items,
        data_components,
    })
...
text = one(text, """        "minecraft:data_component_type": {
...
text = one(text, """        assert_eq!(report.items[1].protocol_id, 1);
...
text = path.read_text()
text = one(text, """    let items = protocol_registries.items;
...
text = path.read_text()
text = one(text, """    BlockPosition, CommonPlayerSpawnInfo, DataComponentProtocolRegistry, DefaultSpawnPosition,
...
""", "runtime tuple names")
text = one(text, """            loaded.registry_payloads,
...
""", """            loaded.registry_payloads,
            ...
            loaded.data_component_protocol_ids,
""", "loaded tuple values")
text = one(text, """            registry_payloads,
...
text = one(text, """struct LoadedVersionPack {
...
text = one(text, """    let item_protocol_ids = ItemProtocolRegistry::new(
...
text = one(text, """    Ok(LoadedVersionPack {
        profile,
        world: pack.world,
        registry_payloads,
        item_protocol_ids,
...
text = path.read_text()
text = one(text, "use ferrum_game::{GameEvent, PLAYER_INVENTORY_SLOTS, PlayerUuid};", "use ferrum_game::{GameEvent, PLAYER_INVENTORY_SLOTS, PlayerUuid};\nuse ferrum_play::EntityProtocolRegistry;", "replication entity import")
text = one(text, """pub struct GameReplicationConfig {
...
text = one(text, """            poll_interval: DEFAULT_POLL_INTERVAL,
        }
