...
if old_import not in worker:
    raise SystemExit("worker import marker not found")
worker = worker.replace(old_import, new_import, 1)

receive_method = '''    pub fn try_recv_output(&self) -> Result<O, WorkerReceiveError> {
...
'''
combined_replacement = receive_method + '''
...
'''
if worker.count(receive_method) != 2:
    raise SystemExit("expected two receive methods")
worker = worker.replace(receive_method, combined_replacement, 1)
worker = worker.replace(receive_method, combined_replacement, 1)

control_marker = '''#[derive(Debug, Clone, Copy, PartialEq, Eq, Error)]
pub enum WorkerControlError {
'''
wait_support = '''fn recv_output<O>(output: &Receiver<O>) -> Result<O, WorkerWaitError> {
    output
        .recv()
        .map_err(|_| WorkerWaitError::RuntimeDisconnected)
}

fn recv_output_timeout<O>(
    output: &Receiver<O>,
    timeout: Duration,
) -> Result<O, WorkerWaitError> {
...
'''
if control_marker not in worker:
    raise SystemExit("control error marker not found")
worker = worker.replace(control_marker, wait_support + control_marker, 1)
...
worker = worker.replace(test_marker, wait_tests + test_marker, 1)
