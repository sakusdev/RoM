...
use rom_game::{CommandSource, EntityId, GameState, PlayerUuid as GamePlayerUuid, Transform};
use rom_nbt::{Tag, encode_anonymous};
use rom_pack::{RomPack, RomPackPacket, RomPackRegistry, RomPackWorld, read_rompack};
use rom_play::{
...
use rom_protocol::{HandshakeIntent, PacketKind, PacketTable, ProtocolProfile, ProtocolSession};
use rom_runtime::ConnectionId;
use rom_server::{
...
#[cfg(test)]
use rom_world::{BlockMutation, BlockPos, ChunkPos, StaticChunk};
...
const DEFAULT_MOTD: &str = "RoM native Rust server";
...
const DEFAULT_WELCOME_MESSAGE: &str = "RoM native Rust world loaded";
...
fn run(cli: Cli) -> Result<()> {
    ...
    println!(
        "rom-server listening on {} as {} protocol {}",
...
fn load_version_pack(path: &Path, config: &ServerConfig) -> Result<LoadedVersionPack> {
    ...
    let data_component_protocol_ids = DataComponentProtocolRegistry::new(
        pack.data_components
            .iter()
...
fn load_anvil_region_file(
    ...
    region: RegionPos,
    profile: &AnvilChunkConversionProfile,
) -> Result<rom_world::anvil::AnvilRegionLoadReport> {
...
impl Default for ServerConfig {
    fn default() -> Self {
        Self {
            ...
            max_players: 20,
            online_players: 0,
            login_disconnect_message: "RoM native server currently implements status ping only"
                .to_owned(),
...
fn handle_play_protocol<R: Read, W: Write>(
    ...
    if let Err(error) = result {
        ...
        let reason = format!("RoM closed the connection: {error}");
...
fn spawn_live_play_writer(
    ...
    let set_container_content = profile.packets().id(PacketKind::SetContainerContent);
    ...
    spawn_play_writer(
        ...
        move |writer, output| match output {
            ...
            PlayOutput::SetContainerContent {
                ...
                if let Some(packet_id) = set_container_content
                    && let Some(payload) = encode_set_container_content(
                        container_id,
                        state_id,
                        &slots,
                        carried.as_ref(),
            ...
            PlayOutput::SetContainerSlot {
                ...
                if let Some(packet_id) = set_container_slot
                    && let Some(payload) = encode_set_container_slot(
                        container_id,
                        state_id,
                        slot,
                        stack.as_ref(),
            ...
            output => write_live_play_output(writer, packet_ids, output),
        },
    )
}
...
mod tests {
    use super::*;
    use rom_nbt::{NamedTag, TagType, encode_named};
    use rom_world::anvil::{HEADER_BYTES, REGION_EDGE_CHUNKS, SECTOR_BYTES};
    use std::io::Cursor;
    ...
    fn parses_expected_config_argument() {
        let cli = Cli::try_parse_from(["rom-server", "--config", "server.toml"])
    ...
    fn requires_config_argument() {
        assert!(Cli::try_parse_from(["rom-server"]).is_err());
    }
    ...
    fn parses_server_config() {
        ...
        assert_eq!(config.motd, "RoM test");
    ...
    fn status_json_includes_vanilla_server_list_metadata() {
        ...
        assert_eq!(status["description"]["text"], "RoM status");
    ...
    fn answers_status_request_and_ping() {
        ...
        assert!(status.contains("RoM status"));
    ...
    fn builtin_profile_disconnects_when_core_pack_is_declined() {
        ...
        assert_eq!(
            rom_nbt::decode_anonymous(disconnect_reader.take_remaining()).unwrap(),
            Tag::String(
    ...
    fn temp_region_file(name: &str) -> PathBuf {
        ...
        path.push(format!("rom-server-{}-{name}", std::process::id()));
        path
    }
