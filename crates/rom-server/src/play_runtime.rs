...
use rom_protocol::{PacketDirection, PacketKind, ProtocolPhase, ProtocolProfile, ProtocolSession};
...
fn decode_hotbar_selection(reader: &mut PacketReader<'_>) -> Result<u8> {
    ...
    if selected >= rom_game::HOTBAR_SLOTS {
        bail!(
            "selected hotbar slot {selected} is outside 0..{}",
            rom_game::HOTBAR_SLOTS
        );
    }
    Ok(selected)
}
...
mod tests {
    ...
    use rom_protocol::PacketTable;
    use rom_runtime::{BoundedInputQueue, worker_channel};
    use rom_server::play_connection::register_play_connection;
    use rom_world::{BlockMutation, BlockPos};
    use std::{
        io::{self, Cursor, Read},
        num::NonZeroUsize,
    };
    ...
    fn play_session() -> ProtocolSession {
        ...
        session
            .handshake(1, rom_protocol::HandshakeIntent::Login)
            .unwrap();
