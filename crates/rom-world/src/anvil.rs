...
use rom_nbt::{DecodeLimits, NamedTag, NbtError, Tag, decode_named_with_limits};
use thiserror::Error;
...
mod tests {
    ...
    use rom_nbt::{NamedTag, Tag, encode_named};
    use std::io::Write;
    ...
    fn summarizes_modern_chunk_nbt_metadata() {
        ...
        root.insert(
            "sections".to_owned(),
            Tag::List {
                element_type: rom_nbt::TagType::Compound,
                elements: vec![
    ...
    fn biome_container(palette: Vec<Tag>, data: Option<Vec<i64>>) -> BTreeMap<String, Tag> {
        ...
        container.insert(
            "palette".to_owned(),
            Tag::List {
                element_type: rom_nbt::TagType::String,
                elements: palette,
            },
        );
        if let Some(data) = data {
