...
impl Ingredient {
    pub fn new(items: Vec<String>) -> Result<Self, CraftingError> {
        ...
        for v in &items {
            if !validate_resource_location(v) {
                return Err(CraftingError::InvalidId(v.clone()));
            }
        }
        Ok(Self(items))
    }
...
impl ShapelessRecipe {
    pub fn new(
        ...
        if !validate_resource_location(&id) {
            return Err(CraftingError::InvalidId(id));
        }
    ...
    #[must_use]
    pub fn matches(&self, g: &CraftingGrid) -> bool {
        let stacks = g
            .slots
            .iter()
        ...
        for ing in &self.ingredients {
            let Some(i) = stacks
                .iter()
                .enumerate()
                .position(|(i, s)| !used[i] && ing.matches(s))
            else {
                return false;
            };
            used[i] = true;
...
mod tests {
    ...
    fn shapeless() {
        let r = ShapelessRecipe::new(
            "rom:test".into(),
            vec![Ingredient::new(vec!["minecraft:stone".into()]).unwrap()],
            ItemStack::new("minecraft:dirt", 1).unwrap(),
        )
        .unwrap();
        let mut g = CraftingGrid::new(2, 2).unwrap();
        g.set(2, Some(ItemStack::new("minecraft:stone", 1).unwrap()))
            .unwrap();
        assert!(r.matches(&g));
    }
}
