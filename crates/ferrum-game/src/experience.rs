...
pub fn level_from_total(total: u64) -> u32 {
    let mut low = 0u32;
    let mut high = 1u32;
    while total_points_for_level(high) <= total && high < 1_000_000 {
...
pub fn progress_from_total(total: u64) -> (u32, f32) {
    ...
    let progress = ((total - base) as f64 / needed as f64).clamp(0.0, 1.0) as f32;
    (level, progress)
}
#[must_use]
...
mod tests {
    ...
    #[test]
    fn inverse_level() {
        for level in [0, 1, 15, 16, 30, 31, 100] {
