...
use crate::{
    EntityError, EntityId, EntityStore, EntityType, EntityUuid, Inventory, InventoryError,
    ItemStack, Transform, Velocity,
};
...
pub fn set_item_entity_data(
    store: &mut EntityStore,
    id: EntityId,
    data: ItemEntityData,
) -> Result<(), ItemEntityError> {
    let entity = store
        .get_mut(id)
        .ok_or(ItemEntityError::UnknownEntity { id })?;
    if entity.entity_type.as_str() != "minecraft:item" {
        return Err(ItemEntityError::NotItemEntity { id });
    }
    entity
        .data
        .insert(ITEM_ENTITY_DATA_KEY.to_owned(), serde_json::to_value(data)?);
    Ok(())
}

pub fn item_entity_in_pickup_range(
    store: &EntityStore,
    id: EntityId,
    player_position: [f64; 3],
) -> Result<bool, ItemEntityError> {
    let entity = store.get(id).ok_or(ItemEntityError::UnknownEntity { id })?;
    if player_position.into_iter().any(|v| !v.is_finite()) {
        return Err(ItemEntityError::NonFinitePickupPosition);
    }
    Ok(distance_squared(entity.transform.position, player_position)
...
pub fn try_pickup_item_entity(
    ...
    if !item_entity_in_pickup_range(store, id, player_position)? {
        return Ok(None);
    }
    let Some(data) = item_entity_data(store, id)? else {
        return Ok(None);
    };
    if !data.can_be_picked_up() {
        return Ok(None);
    }
    ...
    if inserted == 0 {
        return Ok(Some(ItemPickupResult {
            entity_id: id,
            inserted: 0,
            changed_slots,
            remainder,
...
pub fn try_merge_item_entities(
    store: &mut EntityStore,
    first: EntityId,
    second: EntityId,
) -> Result<Option<ItemMergeResult>, ItemEntityError> {
    if first == second {
        return Ok(None);
    }
    let first_entity = store
        .get(first)
        .ok_or(ItemEntityError::UnknownEntity { id: first })?;
    let second_entity = store
        .get(second)
        .ok_or(ItemEntityError::UnknownEntity { id: second })?;
    if distance_squared(
        first_entity.transform.position,
        second_entity.transform.position,
    ) > ITEM_MERGE_RANGE * ITEM_MERGE_RANGE
    {
        return Ok(None);
    }
    let Some(mut a) = item_entity_data(store, first)? else {
        return Ok(None);
    };
    let Some(mut b) = item_entity_data(store, second)? else {
        return Ok(None);
    };
    if !a.stack.can_merge(&b.stack) || a.stack.remaining_capacity() == 0 {
        return Ok(None);
    }
...
pub fn merge_nearby_item_entities(
    store: &mut EntityStore,
) -> Result<Vec<ItemMergeResult>, ItemEntityError> {
    let mut ids = store
        .iter()
        .filter_map(|(id, e)| (e.entity_type.as_str() == "minecraft:item").then_some(*id))
        .collect::<Vec<_>>();
    ids.sort();
    ...
    for i in 0..ids.len() {
        ...
        for j in (i + 1)..ids.len() {
            if store.get(ids[j]).is_none() {
                continue;
            }
            if let Some(result) = try_merge_item_entities(store, ids[i], ids[j])? {
                results.push(result);
            }
        }
    }
...
pub enum ItemEntityError {
    ...
    #[error("entity {id:?} is not an item entity")]
    NotItemEntity { id: EntityId },
    #[error("item entity {id:?} is missing typed item data")]
    MissingItemData { id: EntityId },
    #[error("pickup position is not finite")]
...
mod tests {
    use super::*;
    fn transform(position: [f64; 3], on_ground: bool) -> Transform {
        Transform::new(position, 0.0, 0.0, on_ground).unwrap()
    }
    #[test]
    fn spawns_typed_item_entity() {
        ...
        let id = spawn_item_entity_with_default_delay(
            &mut s,
            EntityUuid::new(1),
            transform([1.0, 64.0, 2.0], false),
            Velocity::default(),
            ItemStack::new("minecraft:stone", 3).unwrap(),
        )
        .unwrap();
        assert_eq!(item_entity_data(&s, id).unwrap().unwrap().stack.count(), 3);
    }
    #[test]
    fn pickup_waits_for_delay() {
        let mut s = EntityStore::new();
        let id = spawn_item_entity(
            &mut s,
            EntityUuid::new(2),
            transform([0.0; 3], true),
            Velocity::default(),
            ItemStack::new("minecraft:stone", 3).unwrap(),
            1,
        )
        ...
        assert!(
            try_pickup_item_entity(&mut s, id, [0.0; 3], &mut inv)
                .unwrap()
                .is_none()
        );
        tick_item_entities(&mut s).unwrap();
        assert_eq!(
            try_pickup_item_entity(&mut s, id, [0.0; 3], &mut inv)
                .unwrap()
                .unwrap()
    ...
    #[test]
    fn partial_pickup_leaves_remainder() {
        let mut s = EntityStore::new();
        let id = spawn_item_entity(
            &mut s,
            EntityUuid::new(3),
            transform([0.0; 3], true),
            Velocity::default(),
            ItemStack::new("minecraft:stone", 64).unwrap(),
            0,
        )
        .unwrap();
        let mut inv = Inventory::new();
        for slot in 9..=44 {
            inv.set_slot(slot, Some(ItemStack::new("minecraft:dirt", 64).unwrap()))
                .unwrap();
        }
        inv.set_slot(9, Some(ItemStack::new("minecraft:stone", 60).unwrap()))
            .unwrap();
        let result = try_pickup_item_entity(&mut s, id, [0.0; 3], &mut inv)
            .unwrap()
            .unwrap();
        assert_eq!(result.inserted, 4);
        assert_eq!(result.remainder.unwrap().count(), 60);
        assert!(s.get(id).is_some());
    }
    #[test]
    fn nearby_stacks_merge() {
        let mut s = EntityStore::new();
        let a = spawn_item_entity(
            &mut s,
            EntityUuid::new(4),
            transform([0.0; 3], true),
            Velocity::default(),
            ItemStack::new("minecraft:stone", 60).unwrap(),
            0,
        )
        .unwrap();
        let b = spawn_item_entity(
            &mut s,
            EntityUuid::new(5),
            transform([0.1, 0.0, 0.0], true),
            Velocity::default(),
            ItemStack::new("minecraft:stone", 4).unwrap(),
            0,
        )
        .unwrap();
        let r = try_merge_item_entities(&mut s, a, b).unwrap().unwrap();
        assert_eq!(r.moved, 4);
        assert_eq!(item_entity_data(&s, a).unwrap().unwrap().stack.count(), 64);
    ...
    fn expires_after_lifetime() {
        ...
        .unwrap();
        s.get_mut(id).unwrap().age_ticks = ITEM_ENTITY_LIFETIME_TICKS;
        assert_eq!(tick_item_entities(&mut s).unwrap(), vec![id]);
    }
}
