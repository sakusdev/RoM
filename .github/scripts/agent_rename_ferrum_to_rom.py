...
# The porting-tool CLI used a bare Ferrum package/binary/command name, so it needs
# an explicit migration in addition to the ferrum-* prefix replacements.
...
cli_main = ROOT / "crates" / "rom-cli" / "src" / "main.rs"
main_source = cli_main.read_text(encoding="utf-8")
main_source = main_source.replace('#[command(name = "ferrum",', '#[command(name = "rom-cli",')
cli_main.write_text(main_source, encoding="utf-8")
...
# Guardrails: old internal crate/package identifiers and paths must be gone. Cargo.lock
# is regenerated by the validation workflow immediately after this migration.
allowed = {
...
for path in ROOT.rglob("*"):
    ...
    try:
        ...
        continue
    if "ferrum-" in text or "ferrum_" in text or 'name = "ferrum"' in text or 'name = "ferrum",' in text:
