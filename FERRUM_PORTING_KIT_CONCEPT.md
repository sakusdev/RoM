# Ferrum Porting Kit — Project Concept and Codex Handoff

## 0. Document purpose

This document defines the concept, architecture, current status, development policy, and immediate implementation tasks for **Ferrum Porting Kit**.

Ferrum Porting Kit is an experimental Rust toolchain intended to accelerate the clean and testable reimplementation of Minecraft Java Edition server behavior in Rust.
...
Ferrum is successful even before full source conversion exists.

The first meaningful success condition is:

> Given a Minecraft server JAR or Fabric Mod JAR, Ferrum can produce a reliable inventory showing which code can probably be ported automatically, which code requires review, and which code requires manual redesign.
...
Ferrum should be distributed as original tooling, runtime support, rewrite rules, tests, and independently written server code.
...
Generic JVM conversion is insufficient. Ferrum requires a configurable rewrite system.
...
"net/minecraft/util/Identifier" = "ferrum_types::Identifier"
"net/minecraft/util/math/BlockPos" = "ferrum_types::BlockPos"
"net/minecraft/util/math/ChunkPos" = "ferrum_types::ChunkPos"
"net/minecraft/item/ItemStack" = "ferrum_inventory::ItemStack"
```
...
"java/lang/System.nanoTime()J" = "ferrum_time::monotonic_nanos"
"java/util/Objects.requireNonNull(Ljava/lang/Object;)Ljava/lang/Object;" = "ferrum_compat::require_non_null"
```
...
Ferrum should support symbol mapping as a separate layer above generic JVM import.
...
Ferrum may analyze Fabric Mod JARs, but complete Fabric compatibility is not an immediate target.
...
Ferrum Porting Kit is not itself the final server.
...
Ferrum-generated knowledge and tests should help implement this architecture, not prevent it.
...
1. Add descriptor and reference models to `ferrum-model`.
2. Create `ferrum-bytecode` or add a temporary internal module to `ferrum-importer`.
...
Ferrum Porting Kit is best understood as:
...
> A clean Rust-native Minecraft server whose behavior is validated against the Java reference implementation, with Ferrum automating the expensive analysis and migration work required to build it.
